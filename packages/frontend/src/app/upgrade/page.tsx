...
export default function UpgradeScreen() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Upgrade failed. Please check your internet connection and try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const features = [
    {
      ...
      description: "Get full access to all practice questions and detailed explanations across every subject.",
    },
    {
      title: "Unlimited Timed Exams",
      description: "Simulate real exam conditions with flexible, customizable timers.",
    },
    {
      title: "Comprehensive Analytics",
      description: "Track your progress and pinpoint subject areas that need more attention.",
    },
    {
      title: "Ad-Free & Priority Load",
      description: "Enjoy faster response times and a fully focused study environment.",
    },
  ];
      ...
      <div className="flex flex-col items-center justify-center h-full px-6 text-center bg-slate-50 gap-6 py-12 animate-scale-in">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-2 max-w-xs leading-relaxed">
            All premium features, unlimited timed exams, and detailed analytics have been successfully unlocked!
          </p>
        </div>
        <button
          onClick={goBack}
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-y-auto px-4 py-6 pb-24">
      ...
      <div className="bg-white rounded-3xl border border-slate-150 p-6 flex flex-col items-center text-center shadow-sm mb-6">
        ...
        <p className="text-xs text-slate-500 mt-1 max-w-xs leading-relaxed">
          Unlock the light of tomorrow and pass your exam with confidence.
        </p>
        ...
        <div className="mt-5 px-5 py-2.5 rounded-full bg-amber-50 border border-amber-200 text-amber-900 flex items-center gap-1">
          ...
          <span className="text-[10px] uppercase font-semibold text-amber-700">/ One-time lifetime access</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 items-center text-[10px] text-slate-400 font-medium justify-center mb-6">
        <ShieldAlert className="w-3.5 h-3.5" /> Safe checkout. Cancel anytime.
      </div>
      ...
      >
        {loading ? (
          ...
          <>
            <Crown className="w-5 h-5 fill-white" /> Activate Premium Now
          </>
        )}
      </button>
    </div>
